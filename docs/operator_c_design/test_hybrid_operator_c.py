...
def ltn_refiner(simple_embedding_fn):
    ...
    config = LTNConfig(
        learning_rate=0.05,  # Higher LR for faster test convergence
        max_iterations=50,
...
class TestLTNRefiner:
    ...
    def test_initialization(self, ltn_refiner):
        ...
        assert ltn_refiner.config.learning_rate == 0.05
        assert ltn_refiner.config.max_iterations == 50
    ...
    def test_simple_refinement(self, ltn_refiner, manifold):
        ...
        assert 0 < sim_to_current < 0.95  # Moved away from current
    ...
    def test_energy_barrier_rejection(self, ltn_refiner, manifold):
        ...
        manifold.store(stored)

        # Create belief and evidence VERY far apart
        current = stored[0].clone()
        evidence = -stored[0]  # Opposite direction (high barrier)

        # Configure LTN with strict energy barrier
        ltn_refiner.config.max_energy_barrier = 1.0  # Very strict
...
class TestFullPipeline:
    ...
    def test_scaffolding_effect(self, hybrid_search, manifold):
        ...
        if result1 is not None and result1.strategy == SearchStrategy.LTN:
            ...
            current2 = result1.pattern + 0.05 * torch.randn(128)
    ...
    def test_belief_revision_workflow(
        ...
        manifold.store(belief_emb.unsqueeze(0))
        ...
        if result is not None:
            ...
            revised_sim = F.cosine_similarity(
                result.pattern, evidence_emb, dim=0
            ).item()
...
class TestStressCases:
    ...
    def test_degenerate_case_identical_inputs(self, hybrid_search):
        ...
        if result is not None:
            sim = F.cosine_similarity(result.pattern, current, dim=0).item()
...
def test_ltn_config_validation():
    ...
    bad_config = LTNConfig(weight_evidence=0.1)
