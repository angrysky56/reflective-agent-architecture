...
class SheafAnalyzer:
    ...
    def build_coboundary_matrix(
        ...
        # Sanity check for NaNs in weights
        for idx, w in enumerate(weights):
            if torch.isnan(w).any() or torch.isinf(w).any():
                logger.warning(f"NaN/Inf detected in weight matrix {idx}. Replacing with zeros.")
                weights[idx] = torch.nan_to_num(w, nan=0.0, posinf=1e6, neginf=-1e6)
