...
class HybridSearchStrategy:
    ...
    def search(
        ...
        result = HybridSearchResult(
            pattern=current_state,  # Placeholder
            energy=0.0,
            distance=0.0,
            source="none",
    ...
    def _try_knn_search(
        ...
        if self.config.require_sheaf_validation and self.sheaf is not None:
            if not self._sheaf_validates(knn_result.pattern):
        ...
        hybrid_result = HybridSearchResult(
            pattern=knn_result.pattern,
            energy=knn_result.energy,
            distance=knn_result.distance,
            source=knn_result.source,
    ...
    def _sheaf_validates(self, pattern: torch.Tensor) -> bool:
        ...
        # Heuristic: Check if in attractor (negative energy)
        energy = self.manifold.energy(pattern).item()

        # Attractors have negative energy in Hopfield networks
