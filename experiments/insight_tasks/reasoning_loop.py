...
class ReasoningConfig:
    ...
    energy_threshold: float = 0.1  # Accept solution if energy below this
    convergence_tolerance: float = 1e-4  # State change threshold for convergence
...
class RAAReasoningLoop:
    ...
    def reason_step(self, current_state: torch.Tensor, step: int) -> tuple[torch.Tensor, dict]:
        ...
        # 2. Compute pseudo-logits for Director
        pseudo_logits = self._compute_pseudo_logits(current_state)

        # 3. Check if energy is low enough (solution found)
    ...
    def reason(
        ...
        Returns:
            solution_state: Final reasoning state (embedding)
