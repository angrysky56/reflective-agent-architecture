...
class ModernHopfieldNetwork(nn.Module):
    ...
    def store_pattern(self, pattern: torch.Tensor, metadata: Optional[dict] = None) -> None:
        ...
        # Robustness Check 1: Check for NaNs or Infs in input
        if torch.isnan(pattern).any() or torch.isinf(pattern).any():
            import logging

            logger = logging.getLogger(__name__)
            logger.warning("Attempted to store pattern with NaNs or Infs. Skipping storage.")
            return

        # Robustness Check 2: Check for Zero Vector (prevents 0/0 -> NaN in normalize)
        norm = torch.norm(pattern, p=2, dim=-1, keepdim=True)
        if (norm < 1e-9).all():
            import logging

            logger = logging.getLogger(__name__)
            logger.warning("Attempted to store Zero Vector pattern. Skipping storage.")
            return

        # Normalize pattern (important for energy landscape stability)
        # Use simple manual normalization to avoid f.normalize issues with near-zero vectors if any slipped through
        pattern = pattern / (norm + 1e-9)
    ...
    def energy(self, state: torch.Tensor) -> torch.Tensor:
        ...
        # Clamp similarities to avoid overflow in exp
        similarities = torch.clamp(similarities, max=50.0)
        ...
        # Robustness Check: Replace NaNs/Infs with fallback energy (0.0)
    ...
    def update_step(self, state: torch.Tensor) -> torch.Tensor:
        ...
        # Check for empty patterns
        if self.patterns.shape[0] == 0:
            return state.squeeze(0) if squeeze_output else state
