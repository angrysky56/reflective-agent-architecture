...
class LTNRefiner:
    ...
    def refine(
        ...
        for iteration in range(self.config.max_iterations):
            ...
            # Normalize for loss calculation (project to sphere)
            cand_norm = f.normalize(candidate, p=2, dim=0)

            # 1. Distance constraint (stay close to current)
            # Use cosine distance on sphere instead of Euclidean
            l_dist = 1.0 - f.cosine_similarity(cand_norm.unsqueeze(0), current.unsqueeze(0), dim=1)

            # 2. Evidence constraint (pull towards evidence)
            l_ev = 1.0 - f.cosine_similarity(cand_norm.unsqueeze(0), target.unsqueeze(0), dim=1)

            # 3. Energy constraint (avoid high energy barriers)
            ...
            # Project back to sphere to prevent gradient vanishing
            with torch.no_grad():
                candidate.data = f.normalize(candidate.data, p=2, dim=0)
