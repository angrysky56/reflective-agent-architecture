...
def manifold(embedding_dim, device):
    ...
    config = HopfieldConfig(
        embedding_dim=embedding_dim, beta=1.0, max_patterns=100, update_steps=5, device=device
    )
    manifold = Manifold(config=config)

    # Store some prototype patterns (clustered with increased diversity)
    num_clusters = 5
    ...
    for cluster_id in range(num_clusters):
        ...
        for _ in range(patterns_per_cluster):
            noise = torch.randn(embedding_dim) * 0.3  # Increased noise for better diversity
...
def director(manifold, device):
    ...
    config = DirectorConfig(
        entropy_threshold_percentile=0.70, search_k=3, use_energy_aware_search=True, device=device
    )
...
def reasoning_loop(manifold, director, pointer, device):
    ...
    config = ReasoningConfig(
        max_steps=10, max_reframing_attempts=3, energy_threshold=0.1, device=device
    )
    return RAAReasoningLoop(manifold=manifold, director=director, pointer=pointer, config=config)
...
def test_reasoning_loop_single_step(reasoning_loop, embedding_dim):
    ...
    assert new_state.shape[-1] == embedding_dim  # Check embedding dimension, not batch size
...
def test_reasoning_loop_full_cycle(reasoning_loop, embedding_dim):
    ...
    solution, metrics = reasoning_loop.reason(
        input_embeddings=input_embedding, return_trajectory=False
    )

    # Verify solution
    assert solution is not None
    assert solution.shape[-1] == embedding_dim  # Check embedding dimension
...
def test_reasoning_loop_metrics_tracking(reasoning_loop, embedding_dim):
    ...
    solution, metrics = reasoning_loop.reason(
        input_embeddings=input_embedding, return_trajectory=True  # Request full trajectory
    )
...
def test_reasoning_loop_energy_threshold(reasoning_loop, embedding_dim):
    ...
    # Set very low energy threshold (unreachable, should never stop early on energy)
    # Typical energies are in range [-2.0, 0.5], so -10.0 is unreachable
    reasoning_loop.config.energy_threshold = -10.0
    # Also increase convergence tolerance to prevent early convergence
    reasoning_loop.config.convergence_tolerance = 0.1
    ...
    # Should stop quickly when energy threshold is easily reached
    assert metrics["total_steps"] <= 3  # Should stop fast with high threshold
    if "convergence_reason" in metrics:
        # Should stop due to energy threshold with this high threshold
        assert metrics["convergence_reason"] == "energy_threshold"
...
def test_reasoning_loop_component_integration(manifold, director, pointer):
    ...
    loop = RAAReasoningLoop(manifold=manifold, director=director, pointer=pointer)
