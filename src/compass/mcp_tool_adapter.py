...
async def get_available_tools_for_llm(mcp_client: Any) -> List[Dict]:
    ...
    try:
        ...
        logger.info(
            f"MCP Tool Adapter: Calling get_available_tools() on {type(mcp_client).__name__}"
        )
        ...
        if hasattr(mcp_client, "external_mcp") and mcp_client.external_mcp:
            logger.info(
                f"MCP Tool Adapter: external_mcp is_initialized={mcp_client.external_mcp.is_initialized}"
            )
            logger.info(
                f"MCP Tool Adapter: external_mcp tools_map has {len(mcp_client.external_mcp.tools_map)} tools"
            )
