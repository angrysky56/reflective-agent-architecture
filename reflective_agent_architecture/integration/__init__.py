...
Usage:
    ...
    # Core RAA reasoning
    from reflective_agent_architecture.integration import RAAReasoningLoop, ReasoningConfig

    loop = RAAReasoningLoop(manifold, director, pointer)
    solution, metrics = loop.reason(input_embeddings)

    # CWD-RAA integration
    from reflective_agent_architecture.integration import CWDRAABridge, EmbeddingMapper
