...
from .utils import COMPASSLogger, Goal, calculate_benefit, calculate_cost, confidence_from_delta_mu, update_precision, validate_non_negative, validate_positive
...
class oMCDController:
    ...
    def determine_resource_allocation(self, current_state: Dict, importance: float, available_resources: float) -> Dict[str, float]:
        """
        ...
        optimal_z = self._calculate_optimal_allocation(initial_precision, value_difference, variance, importance, available_resources)
        ...
        allocation = {"amount": optimal_z, "confidence": confidence, "benefit": benefit, "cost": cost, "net_benefit": benefit - cost, "updated_precision": updated_precision}

        self.decision_history.append(allocation)

        self.logger.debug(f"Resource allocation: z={optimal_z:.2f}, confidence={confidence:.3f}, net_benefit={benefit - cost:.2f}")

        return allocation

    def _calculate_optimal_allocation(self, initial_precision: float, value_difference: float, variance: float, importance: float, max_resources: float) -> float:
        """
        ...
        for z in np.linspace(0, max_resources, 100):
            ...
            conf = confidence_from_delta_mu(value_difference, 1.0 / precision, self.config.lambda_conf)
